...
some duplicates in-place such that each unique
...
with the first five elements of nums being 1, 1, 2, 2 and
3 respectively. It does not matter what you leave beyond
...
def removeDuplicates(nums):
    ...
    :rtype: int
    """
    return remove_duplicates(nums, {}, 0, 0)


def remove_duplicates(nums, dictionary, ind, k):
    if ind == len(nums):
        return k
    new_k = k
    current = nums[ind]
    ...
    if current in dictionary.keys():
        ...
        new_k += 1
        new_ind += 1
    return remove_duplicates(nums, dictionary, new_ind, new_k)
